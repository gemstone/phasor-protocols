...
public sealed class MultiProtocolFrameParser : IFrameParser
{
    ...
    private sealed class SharedTcpServerReference : IServer
    {
        ...
        /// </summary>
        public string Name => m_tcpServer?.Name ?? "Undefined";
        ...
        private void SharedServer_ClientConnected(object? sender, EventArgs<Guid> e)
        {
            ...
            if (client?.Provider?.RemoteEndPoint is not IPEndPoint remoteEndPoint)
                return;
    ...
    private LongSynchronizedOperation? m_readNextBuffer;
    ...
    public MultiProtocolFrameParser()
    {
        ...
        m_rateCalcTimer = s_timerScheduler.CreateTimer();
    ...
    public bool AutoRepeatCapturedPlayback
    {
        ...
        set
        {
            ...
            if (m_transportProtocol != TransportProtocol.File || m_dataChannel is not FileClient fileClient)
                return;

            fileClient.AutoRepeat = value;

            // If at end of file and re-enabling auto-repeat, trigger reading of next buffer
            if (fileClient is { AutoRepeat: true, CurrentState: ClientState.Connected, AtEOF: true })
    ...
    /// </summary>
    public bool IsIEEEProtocol => m_phasorProtocol is 
                                  PhasorProtocol.IEEEC37_118V2 or 
                                  PhasorProtocol.IEEEC37_118V1 or 
                                  PhasorProtocol.IEEEC37_118D6 or 
                                  PhasorProtocol.IEEE1344;
    ...
    public bool IsConnected
    {
        get
        {
            if (m_commandChannel is not null && KeepCommandChannelOpen)
            ...
            if (m_serverBasedDataChannel is not null)
    ...
    private void Dispose(bool disposing)
    {
        ...
        try
        {
            ...
            if (m_rateCalcTimer is not null)
            {
                m_rateCalcTimer.Elapsed -= m_rateCalcTimer_Elapsed;
    ...
    public void Start()
    {
        ...
        catch (SocketException ex)
        {
            ...
            OnConnectionException(new InvalidOperationException(
                ex.ErrorCode == 10014 ? 
                    $"Bad IP address format in \"{m_connectionString}\": {ex.Message}\r\n\r\nUse a DNS name or an IPv6 formatted IP address (e.g., ::1); otherwise, force IPv4 mode." : 
                    $"{ex.Message} in \"{m_connectionString}\"", ex), 1);
        }
        catch (Exception ex)
        {
            Stop();
    ...
    private void InitializeFrameParser(Dictionary<string, string> settings)
    {
        ...
        if (ReceivedFrameBufferImage is not null)
    ...
    private void InitializeCommandChannel(string connectionString)
    {
        ...
        // Verify user did not attempt to set up command channel as a TCP server
    ...
    private void InitializeDataChannel(Dictionary<string, string> settings)
    {
        ...
        switch (m_transportProtocol)
        {
            case TransportProtocol.Tcp:
                ...
                if (settings.TryGetValue("isListener", out string? setting))
                {
                    if (setting.ParseBoolean())
                        m_serverBasedDataChannel = settings.ContainsKey("receiveFrom") ? new SharedTcpServerReference() : new TcpServer();
                    else
            ...
            case TransportProtocol.File:
                ...
                m_readNextBuffer = new LongSynchronizedOperation(ReadNextFileBuffer, ex => OnParsingException(new InvalidOperationException($"Encountered an exception while reading file data: {ex.Message}", ex)));
                break;
            default:
        ...
        else if (m_serverBasedDataChannel is not null)
        {
            // Setup event handlers
            ...
            m_serverBasedDataChannel.ReceiveClientDataComplete += m_serverBasedDataChannel_ReceiveClientDataComplete;
    ...
    {
        ArgumentNullException.ThrowIfNull(commandFrame);
        ...
        else if (m_dataChannel is not null && m_dataChannel.CurrentState == ClientState.Connected)
        {
            handle = m_dataChannel.SendAsync(buffer, 0, buffer.Length);
        }
        else if (m_serverBasedDataChannel is not null && m_serverBasedDataChannel.CurrentState == ServerState.Running)
        {
    ...
    private void m_dataChannel_ReceiveDataFrom(object? sender, EventArgs<EndPoint, IPPacketInformation, int> e)
    {
        ...
        if (e.Argument1 is not IPEndPoint remoteEndPoint)
            return;
        ...
        if (destinationAddress is not null && m_multicastServerAddress is not null && !destinationAddress.Equals(m_multicastServerAddress))
            return;
    ...
    private void m_serverBasedDataChannel_ReceiveClientDataComplete(object? sender, EventArgs<Guid, byte[], int> e)
    {
    ...
    private void m_frameParser_ReceivedChannelFrame(object? sender, EventArgs<IChannelFrame, bool> e)
    {
        ...
        try
        {
            ...
            if (InjectSimulatedTimestamp && (frame is not IDataFrame || m_transportProtocol != TransportProtocol.File))
    ...
    private void m_frameParser_ReceivedConfigurationFrame(object? sender, EventArgs<IConfigurationFrame> e)
    {
        ...
        if (!KeepCommandChannelOpen && m_commandChannel is not null && m_commandChannel.CurrentState == ClientState.Connected)
            m_commandChannel.Disconnect();
    }
